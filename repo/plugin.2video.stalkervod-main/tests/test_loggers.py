...
class TestLogger(unittest.TestCase):
    ...
    def test_info(self, mock_xbmc):
        ...
        with patch.object(mock_xbmc, 'LOGINFO', 1):
            Logger.info('test')
            mock_xbmc.log.assert_called_with('plugin.2video.stalkervod: test', 1)
    ...
    def test_error(self, mock_xbmc):
        ...
        with patch.object(mock_xbmc, 'LOGERROR', 4):
            Logger.error('test')
            mock_xbmc.log.assert_called_with('plugin.2video.stalkervod: test', 4)
    ...
    def test_warn(self, mock_xbmc):
        ...
        with patch.object(mock_xbmc, 'LOGWARNING', 3):
            Logger.warn('test')
            mock_xbmc.log.assert_called_with('plugin.2video.stalkervod: test', 3)
    ...
    def test_debug(self, mock_xbmc):
        ...
        with patch.object(mock_xbmc, 'LOGDEBUG', 0):
            Logger.debug('test')
            mock_xbmc.log.assert_called_with('plugin.2video.stalkervod: test', 0)
