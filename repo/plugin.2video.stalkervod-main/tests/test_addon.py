...
class TestStalkerAddon(unittest.TestCase):
    ...
    @patch('sys.argv', ['plugin://plugin.2video.stalkervod/', '1'])
    ...
    def test_run(self, mock_api, mock_xbmc):
        """Test run"""
        run(['plugin://plugin.2video.stalkervod/', '1', '?action=add_fav&video_id=1234&_type=vod'])
    ...
    def test_search_vod(self, mock_ask_for_input, mock_xbmc):
        ...
        self.stalker_addon.router(params)
        mock_xbmc.executebuiltin.assert_called_with('Container.Update(plugin://plugin.2video.stalkervod/?action=vod_listing&category=English+Movies&update_listing=False&search_term=search_term&page=1)')
    ...
    def test_search_tv(self, mock_ask_for_input, mock_xbmc):
        ...
        self.stalker_addon.router(params)
        mock_xbmc.executebuiltin.assert_called_with('Container.Update(plugin://plugin.2video.stalkervod/?action=tv_listing&category=English+Movies&update_listing=False&search_term=search_term&page=1)')
    ...
    def test_search_series(self, mock_ask_for_input, mock_xbmc):
        ...
        self.stalker_addon.router(params)
        mock_xbmc.executebuiltin.assert_called_with('Container.Update(plugin://plugin.2video.stalkervod/?action=series_listing&category=English+Movies&update_listing=False&search_term=search_term&page=1)')
