...
class StalkerPortal:
	...
	def generate_headers(self, include_auth= True, include_token= True, custom_headers= None):
		...
		if include_auth and self.__token.value: headers["Authorization"] = "Bearer %s" % self.__token.value
	...
	def check(self):
		try:
			...
			if int(status) > 399:
				log("function check Stream: %s" % status)
...
def check_portal(url, maclist, silent=False):
	...
	if cacheOk and vav != url:
		del_cache("stalker_groups")
