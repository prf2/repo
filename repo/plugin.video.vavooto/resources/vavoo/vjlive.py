	...
	if not cacheOk:
		...
		if check == True: pass
			...
			if m == False:
				...
				setSetting("stalker", "false")
				return {}
		mac, stalkerurl = get_cache_or_setting("mac"), get_cache_or_setting("stalkerurl")
		portal = StalkerPortal(stalkerurl, mac)
		chan = portal.channels()
		set_cache("sta_channels" , chan, timeout=60* int(getSetting("stalk_cache")))
	sta_channels= {}
	for item in chan:
...
def livePlay(name, type=None, group=None):
	...
	if "hls" in url or "m3u8" in url:
		inputstream = "inputstream.ffmpegdirect" if getSetting("hlsinputstream") == "0" else "inputstream.adaptive"
