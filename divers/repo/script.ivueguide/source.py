...
timeshift = translatePath('special://profile/addon_data/script.ivueguide/timezone.ini')
path_to_settings = translatePath('special://profile/addon_data/script.ivueguide')
current_db = translatePath('special://profile/addon_data/script.ivueguide/program.db')
temp_db = translatePath('special://profile/addon_data/script.ivueguide/temp.db')
...
class Database(object):
    ...
    def __init__(self):
        ...
        self.streamlist = guideurl('YWRkb24ucHk=', 14176)
    ...
    def _updateChannelAndProgramListCaches(self, date, progress_callback, clearExistingProgramList):
        ...
        try:
            ...
            if not os.path.exists(current_db):
                ...
                c.execute("ATTACH DATABASE ? AS db2", (temp_db,))                
                ...
                c.execute("DROP TABLE db2.programs")     
    ...
    def _getEPGView(self, channelStart, date, progress_callback, clearExistingProgramList,category):
        try:sec = grabber.currentcatini().has_section(category)
        except: sec = grabber.currentcatini().has_section(category.encode())
        self._updateChannelAndProgramListCaches(date, progress_callback, clearExistingProgramList)
        self.category = category
        if category and category != "All Channels" and sec:
            self.catchan = grabber.currentcatini().options(self.category)
            get = False
        else:
            get = True
        
        channels = self._getChannelList(onlyVisible=True, all=get)
    ...
    def _onNowEpg(self, category):
        ...
        if self.osdcategory == 'All Channels':
            channelIds = [c.id for c in channels]
        else:
            catchan = grabber.currentcatini().options(self.osdcategory)
            channelIds = [c.id for c in channels if c.title in catchan]
        channelMap = dict()
        for cc in channels:
            if cc.id:
                channelMap[cc.id] = cc
    ...
    def getChannelList(self, onlyVisible=True, all=True, notVisible=False, osdCat=False):
    ...
    def _getChannelList(self, onlyVisible, all=True, notVisible=False, osdCat=False):
        ...
        for row in c:
            ...
            if all == False and self.category != 'All Channels':
                if self.catchan:
                    if channel.title in self.catchan:
    ...
    def getChannelINI(self):
        if not os.path.exists(translatePath('special://profile/addon_data/plugin.video.IVUEcreator')):
            os.makedirs(translatePath('special://profile/addon_data/plugin.video.IVUEcreator'))
        channelsList = self.getChannelList(False,True)
        channels = [channel.title for channel in channelsList]
        ini = xbmcvfs.File('special://profile/addon_data/plugin.video.IVUEcreator/custom_channels.ini','w+')
    ...
    def channelSetup(self, append=None):
        setupPath = translatePath('special://profile/addon_data/script.ivueguide/resources/guide_setups')
        ...
        channelsList = self.getChannelList(False,True)
        ini = xbmcvfs.File('special://profile/addon_data/script.ivueguide/resources/guide_setups/%s' % xmltvfile,'w+')
        for channel in channelsList:
            ini.write("%s , Visible = %s , Position = %s\n" % (str(channel.id), str(channel.visible), str(channel.weight)))
        ini.close()
    ...
    def getvodProgram(self, channel):
        return self._invokeAndBlockForResult(self._getvodProgram, channel)

    def _getvodProgram(self, program):
        try:
            vodProgram = None
            c = self.conn.cursor()
            c.execute(
                'SELECT * FROM programs WHERE channel=? AND source=? AND start_date = ?',
                [program.channel.id, self.source.KEY, program.startDate])
            row = c.fetchone()
            if row:
                vodProgram = Program(program.channel, row['title'], row['start_date'], row['end_date'], row['description'], row['categories'],
                                      row['image_large'], row['image_small'], None, row['season'], row['episode'],
                                      row['is_movie'], row['date'], row['language'])
            c.close()
    ...
    def _setCustomStreamUrl(self, channel, stream_url):
        if stream_url is not None:
            c = self.conn.cursor()
            c.execute("UPDATE channels SET stream_url=? WHERE id=?", [stream_url, channel.id])
    ...
    def adapt_datetime(ts):
        ...
        try: timet = time.mktime(ts.timetuple())
        except: timet = time.mktime(ts.utctimetuple())
        return timet
...
class XMLTVSource(Source):
    PLUGIN_DATA = translatePath(os.path.join('special://profile', 'addon_data', 'script.ivueguide'))
    KEY = 'xmltv'
    LOGO_SOURCE_IVUE = 0
    LOGO_SOURCE_SUB = 1
...
#Karls custom xml import for creator
        ...
        if self.xmltvType == gType.CUSTOM_FILE_ID:
            if addon.getSetting('custom.xmltv.type') == "0":
                try:
                    ...
                    pass
                xmltvTemp = translatePath(os.path.join('special://profile', 'addon_data', 'script.ivueguide', 'custom.xml'))
            ...
            if event == "end":
                ...
                if elem.tag == "programme":
                    ...
                    if xmltvType == '':
                        ...
                        xmltvType = ADDON.getSetting('sub.xmltv')
                    xmltvfile = xmltvType + '.ini'
                    inifile = translatePath('special://profile/addon_data/script.ivueguide/resources/guide_setups/%s' % xmltvfile)
                    if os.path.exists(inifile):
                        ini = open(inifile, "rb")
                        mysetup= []
                        readini = ini.readlines()
                        getlist = sorted(readini)
                        for setup in getlist:
                            try:
                                if cid == setup.split(' , Visible')[0]:
                                    visible = setup.split('Visible = ')[1].split(' , Position')[0]
                                    if visible == 'False': visible = '0'
                                    elif visible == 'True': visible = '1'
                                    weight = int(setup.split('Position = ')[1])
                            except:
                                if cid == setup.decode().split(' , Visible')[0]:
                                    visible = setup.decode().split('Visible = ')[1].split(' , Position')[0]
                                    if visible == 'False': visible = '0'
                                    elif visible == 'True': visible = '1'
                                    weight = int(setup.decode().split('Position = ')[1])
    ...
    def parseXMLTVchannels(self, progress_callback):
        self.xml = open(self.xmltvFile, 'rb').read().decode('utf-8')
        ...
        for channel in channels:
            ...
            chanList = [cid, title, logo, streamUrl, 'xmltv', visible, weight]
            yield chanList
        if self.conn:
            self.conn.close()
...
class FileWrapper(object):
    ...
    def read(self, byteCount):
        ...
        if PY2: return self.vfsfile.read(byteCount)
        else: return self.vfsfile.readBytes(byteCount)
...
def tvTimes():
    ...
    channels = []
    try:
        xmlUrl = urlrequest.urlopen(utils.tvtimes()).read().splitlines()
    except:
        pass
    for item in xmlUrl:
